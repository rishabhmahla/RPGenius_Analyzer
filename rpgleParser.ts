...
import * as R from './regexRules';

// ─── Internal Parser State ────────────────────────────────────────────────────

interface ParserState {
  isFreeFormat: boolean;       // True when in **FREE mode
  inInlineFree: boolean;       // True when inside /FREE.../END-FREE block
  inSqlBlock: boolean;         // Collecting multi-line EXEC SQL
  sqlBuffer: string;           // Accumulated SQL text
  sqlStartLine: number;        // Line where EXEC SQL began
  inProcedure: boolean;        // Inside a DCL-PROC block
  currentProc: ProcedureRef | null;
  inDS: boolean;               // Inside a DCL-DS block
  currentDS: DataStructureRef | null;
  inPrototype: boolean;        // Inside a DCL-PR block
  currentPR: PrototypeRef | null;
  inProcInterface: boolean;    // Inside a DCL-PI block
}
 ...
 */
export function parseRpgle(source: string, filePath: string): RpgleProgram {
  ...
  for (let lineNum = 0; lineNum < lines.length; lineNum++) {
    ...
    const loc: SourceLocation = { line: lineNum, rawLine };
    ...
    // Skip comments
    if (isComment(rawLine, isFree)) { continue; }
...
function isComment(line: string, isFree: boolean): boolean {
  if (isFree) {
  ...
  // Check col 7 (* = comment)
  if (stripped.length >= 7 && stripped[6] === '*') { return true; }
  // Check for //-style even in fixed sections (some compilers allow it)
  if (R.RE_FREE_COMMENT.test(line)) { return true; }
  return false;
}

// ─── Free-Format Line Parser ──────────────────────────────────────────────────

function parseFreeFormatLine(
  line: string,
  loc: SourceLocation,
  ...
  if (m) {
    ...
    const dsName = m[1];
    let dsType: DataStructureRef['dsType'] = 'DS';
    if (likeM) { dsType = 'LIKEDS'; }
    else if (isTemplate) { dsType = 'TEMPLATE'; }
    else if (isQualified) { dsType = 'QUALIFIED'; }
...
function parseFixedFormatLine(
  ...
  if (trimmed.toUpperCase().startsWith('/COPY') || trimmed.toUpperCase().startsWith('/INCLUDE')) {
    ...
    if (m) { parseCopybook(m, loc, program); }
    return;
  }
...
function parseFixedFspec(line: string, loc: SourceLocation, program: RpgleProgram): void {
  ...
  if (restLine.includes('DISK'))    { fileType = 'DISK'; }
  if (restLine.includes('WORKSTN')) { fileType = 'WORKSTN'; }
  if (restLine.includes('PRINTER')) { fileType = 'PRINTER'; }
  if (restLine.includes('SEQ'))     { fileType = 'SEQ'; }
  if (restLine.includes('SPECIAL')) { fileType = 'SPECIAL'; }
...
function parseDclF(
  ...
  let fileType: FileType = 'UNKNOWN';
  switch (deviceStr) {
    case 'DISK':    fileType = 'DISK';    break;
    case 'WORKSTN': fileType = 'WORKSTN'; break;
    case 'PRINTER': fileType = 'PRINTER'; break;
    case 'SEQ':     fileType = 'SEQ';     break;
    case 'SPECIAL': fileType = 'SPECIAL'; break;
  }
...
function parseProcParam(trimmed: string): ProcParameter | null {
  ...
  return {
    name,
    type: m[2].trim(),
    direction: m[3] ? (m[3].toUpperCase() as 'CONST' | 'VALUE') : 'REF',
  };
}

function parseDsSubfield(trimmed: string, loc: SourceLocation): SubfieldRef | null {
...
function normalizeUsage(raw: string): FileUsage {
...
function resolveDclFUsage(usageStr: string): FileUsage {
  const hasInput = usageStr.includes('INPUT');
  const hasOutput = usageStr.includes('OUTPUT');
  const hasUpdate = usageStr.includes('UPDATE');

  if (hasUpdate || (hasInput && hasOutput)) { return 'U'; }
  if (hasInput) { return 'I'; }
  if (hasOutput) { return 'O'; }
  return 'UNKNOWN';
}

function isFixedFspecKeyed(restLine: string): boolean {
...
function finalizeOpenBlocks(program: RpgleProgram, state: ParserState, lines: string[]): void {
  if (state.inProcedure && state.currentProc) {
    program.warnings.push({
...
function classifySqlType(sqlText: string): SqlStatement['statementType'] {
  const upper = sqlText.trim().toUpperCase();
  for (const [key, re] of Object.entries(R.RE_SQL_TYPE)) {
    if (re.test(upper)) {
      return key as SqlStatement['statementType'];
    }
  }
  return 'OTHER';
}

function flushSql(program: RpgleProgram, state: ParserState): void {
  const sqlText = state.sqlBuffer.replace(/\s+/g, ' ').trim();
  const stmtType = classifySqlType(sqlText);

  const loc: SourceLocation = { line: state.sqlStartLine, rawLine: 'EXEC SQL ...' };
