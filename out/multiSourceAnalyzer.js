...
exports.detectSourceKind = detectSourceKind;
...
function analyzeSource(content, filePath) {
    ...
    switch (kind) {
        case 'RPGLE': {
            const p = (0, rpgleParser_1.parseRpgle)(content, filePath);
            p.sourceType = 'RPGLE';
            return p;
        }
        case 'SQLRPGLE': {
...
function parseCl(content, filePath, kind) {
    ...
    lines.forEach((line, idx) => {
        const t = line.trim();
        const loc = { line: idx, rawLine: line };
        ...
        if (m) {
            const callType = /^CALLPRC/i.test(t) ? 'CALLP' : 'CALL';
...
function parseDds(content, filePath, kind) {
    const lines = content.split(/\r?\n/);
    const programName = path.basename(filePath).split('.')[0].toUpperCase();
    const program = {
        programName,
...
//# sourceMappingURL=multiSourceAnalyzer.js.map