...
exports.parseDspfContent = parseDspfContent;
exports.renderDspfAsHtml = renderDspfAsHtml;
function parseDspfContent(content) {
...
function parseDspfField(line, payload) {
...
function determineFieldLength(payload, hasName, displayText) {
    if (displayText) {
...
function determineFieldType(payload, hasText, name, hasSpecialText) {
...
function parseDspatr(payload) {
    const attrs = {};
    const dspatrMatch = /\bDSPATR\s*\(\s*([^)]+)\s*\)/i.exec(payload);
    if (dspatrMatch) {
        const attrList = dspatrMatch[1].toUpperCase();
        if (/RI|REVERSE/.test(attrList))
            attrs.reverse = true;
        if (/UL|UNDERLINE/.test(attrList))
            attrs.underline = true;
        if (/BL|BLINK/.test(attrList))
            attrs.blink = true;
        if (/BR|BRIGHT/.test(attrList)) {
            attrs.bright = true;
            attrs.bold = true;
        }
        if (/HI|INVISIBLE/.test(attrList))
    ...
    if (/\bREVERSE\b|\bRI\b/i.test(payload))
        attrs.reverse = true;
    if (/\bUNDERLINE\b|\bUL\b/i.test(payload))
        attrs.underline = true;
    if (/\bBLINK\b|\bBL\b/i.test(payload))
        attrs.blink = true;
    if (/\bBRIGHT\b|\bBR\b/i.test(payload)) {
        attrs.bright = true;
        attrs.bold = true;
    }
    if (/\bINVISIBLE\b|\bHI\b/i.test(payload))
        attrs.invisible = true;
    return attrs;
}
...
function extractColhdg(payload) {
...
function extractEditmask(payload) {
    const match = /EDTMSK\s*\(\s*'([^']*)'\s*\)/i.exec(payload);
    return match ? match[1] : undefined;
}
function extractEditcode(payload) {
    const match = /EDTCDE\s*\(\s*'?([A-Z0-9])'?\s*\)/i.exec(payload);
    return match ? match[1] : undefined;
}
function extractSpecialFieldText(payload) {
...
function extractCondition(payload) {
    const match = /(?:IF|WHEN)\s*\(?\s*([A-Z0-9_#$@]+)\s*\)?/i.exec(payload);
    return match ? match[1] : undefined;
}
function parseRowCol(payload) {
...
function buildRenderPlan(records, selectedName) {
...
function renderDspfAsHtml(content, selectedRecordName) {
    ...
    const attrMatrix = Array.from({ length: ROWS }, () => Array.from({ length: COLS }, () => ({})));
...
function renderRecord(matrix, attrMatrix, record, rowOffset) {
...
function getAttributeClasses(attrs) {
    const classes = [];
    if (attrs.type === 'input')
        classes.push('input');
...
function getAttributeStyle(attrs) {
    const styleParts = [];
    if (attrs.color) {
...
//# sourceMappingURL=dspfRenderer.js.map